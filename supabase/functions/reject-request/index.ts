...
serve(async (req) => {
  ...
  try {
    ...
    if (profileError || (profile?.role !== 'admin' && profile?.role !== 'supervisor')) {
      return new Response(JSON.stringify({ error: 'Solo supervisores pueden rechazar solicitudes' }), {
    ...
    if (!reason || reason.trim().length === 0) {
      return new Response(JSON.stringify({ error: 'reason es requerido' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    // 6. Verificar que el usuario es el supervisor asignado (o admin)
    ...
    // 7. Actualizar la solicitud a rechazada
    ...
    // 9. Notificar al solicitante (aquí iría la lógica de email)
